    ...
    tasks = [
      {
        ...
        prompt: "Let's consider an n × n matrix A whose each row and each column is sorted. That is, A[i, j] ≤ A[i, j+1] and A[i, j] ≤ A[i+1, j]. Given a number x, we want to find whether x exists in this matrix or not. We try to generalize the Binary Search algorithm to 2 dimensions. We use divide-and-conquer (or reduce-and-conquer) approach. Probe the element at the center of the matrix. Based on the result, eliminate some part of the matrix from your search space and focus on the part(s) where your answer could lie." + "\n" +
        "(a) Give a pseudocode and English explanation for this algorithm.",
        additionalPrompt:  "(b) Set up a recurrence for the analysis of running time of this algorithm and obtain the best possible big-O complexity." + "\n" +
      ...
      },
      {
        ...
        prompt: "Write a brute-force algorithm to solve palidromes in Ruby",
        additionalPrompt: "Give an optimized solution",
      }
    ];
